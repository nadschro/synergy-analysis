...
# This will be used to save result files and plot titles. 
# In this example data set we study the perturbation of 4 genes in a hiPSC model of schizophrenia.
...
# Read in metadata (here "Meta_CombinatorialPerturbationProject.csv") 
# and expression data (raw counts, here "Counts_CombinatorialPerturbationProject.csv") 
# and match the order (this step is very important to avoid later confusion).
...
# Plot counts over cpm(counts) and visually inspect the graph. 
# Here, we aim to keep roughly 10 counts in 4 or more samples 
# (number dependent on total number of samples/replicates).
...
# Plot multidimensional scaling plot to assess variables that should be added as covariates.
...
# Define the linear model to be used in the differential expression analysis. 
# Add the variable of interest as well as all variables (metadata columns) 
# that resulted from the visual inspection of the MDS plot in Step 8. 
# In this example “mod.gene” represents our variable of interest, 
# while the MDS plot showed “line” and “donor” as additional covariates. 
...
## 10.	Voom transform 
# The voom() function prepares the data for linear modeling by converting counts to logCPM 
# and computing weights for heteroscedasticity adjustment. 
# It also creates a diagnostic plot for the mean-variance trend. 
# Visually inspect it for fit. Increase the cutoff for lowly expressed genes to improve fit if necessary.
...
# Define your comparisons of interest. Each comparison is assigned a name and its function using elementary algebra. 
# In addition to the standard comparisons of the individual and combined gene perturbations with their control, 
# we also add equations for the additive and the synergistic model.
...
# Create volcano and mean difference (MA) plots for each contrast, 
# with all significant DE genes/ the top 10 genes highlighted, respectively.
...
#"scale_x_dicrete" and "scale_fill_manual" (blue) are commented out and optional additions for aesthetics. 
# *If used, their variable names have to be changed to reflect the data at hand.*
...
# Calculate power. Choose the SE matrix column names that represent the additive and the combinatorial perturbation 
# to calculate the median standard error. Then use them to run the power.compare.logFC() function, which creates a power plot.
colnames(SE)
...
# Calculate synergy coefficient and percentage of synergistic DE genes (FDR < 10%). 
# Plot histogram of all synergistic P-values to visualize the distribution.
...
# Create a table combining log2 fold change columns of additive, combinatorial and synergy contrasts. 
# Make certain the vectors (marked red) refer to the column indices of 
# ensembl ID, gene name, logFC and adjusted P-value in the DEG result tables.
...
# A dd a column assigning synergistic categories to each gene using the categorize.synergy() function, 
# which takes the previously created matrix and the expression cutoff into account.
...
# Create heatmaps of the log2FC in the additive and the combinatorial comparisons for each synergy category.
...
# In this example these are manually curated gene set groups, saved in the “genesets” folder: 
# disorder.gmt, behavior.gmt, connectivity.gmt, head.gmt, neural.gmt, postsynapse.gmt, presynapse.gmt and synapse.gmt.
...
# Loop through all contrasts in the cont.matrix object. Perform enrichment and visualize as scatter 
